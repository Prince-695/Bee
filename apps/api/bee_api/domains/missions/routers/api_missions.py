"""API Missions Router (/api/missions)."""
...
router = APIRouter(prefix="/api/missions", tags=["Missions API"])
_mission_store = MissionStore(settings.DB_PATH)
_orchestrator = MissionOrchestrator(settings.DB_PATH)


class ApiCreateMissionRequest(BaseModel):
    objective: str
    signal_id: Optional[str] = None


@router.post("", status_code=status.HTTP_201_CREATED)
async def create_mission(req: ApiCreateMissionRequest) -> Dict[str, Any]:
